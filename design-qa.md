...
- Проверка выполнена в in-app Browser при явном viewport 1920×1080. Canvas заполняет весь viewport без document overflow; header занимает 16.5% высоты и использует официальный KV-фрагмент с Kaspi.kz и Almaty Marathon.
...
- Kaspi.kz загружается отдельным официальным белым SVG. Almaty Marathon остаётся точным пиксельным фрагментом исходного `kv-original.png`; красный фон фрагмента удаляется на уровне отображения grayscale/contrast + screen blend, поэтому отдельный перерисованный бренд-ассет не создаётся.
